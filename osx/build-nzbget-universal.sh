...
Compile_x86_64()
{
    ./configure --prefix=$(pwd)/tmp/x86 \
        ...
        --with-tlslib=OpenSSL \
        --program-prefix="" \
        CXXFLAGS="-arch x86_64 -std=c++17 -O2 -I$(pwd)/tmp/openssl/x86/include -I$(pwd)/tmp/boost/boost-1.84.0/x86/include" \
...
Compile_arm()
{
    ./configure --prefix=$(pwd)/tmp/arm \
        ...
        --with-tlslib=OpenSSL \
        --program-prefix="" \
        CXXFLAGS="-arch arm64 -std=c++17 -O2 -I$(pwd)/tmp/openssl/arm/include -I$(pwd)/tmp/boost/boost-1.84.0/arm/include" \
