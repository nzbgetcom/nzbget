...
{
	PostUnpackRenamer::Controller renamer;
	...
	BOOST_CHECK_EQUAL(renamer.ResolveSubtitleName("meta", "a.en", ".srt"), "meta.en.srt");
	BOOST_CHECK_EQUAL(renamer.ResolveSubtitleName("meta", "1.fr", ".srt"), "meta.fr.srt");
...
BOOST_AUTO_TEST_CASE(ResolveUniqueNameTest)
{
	...
	std::string name1 = renamer.ResolveUniqueName("meta", "stem", ".mkv", "meta.mkv", usedPaths, workingDir);
	BOOST_CHECK_EQUAL(name1, "meta.mkv");
	usedPaths.insert(workingDir / name1);

	std::string name2 = renamer.ResolveUniqueName("meta", "stem", ".mkv", "meta.mkv", usedPaths, workingDir);
	BOOST_CHECK_EQUAL(name2, "meta(1).mkv");
	usedPaths.insert(workingDir / name2);

	WriteEmptyFile(workingDir / "meta(2).mkv");
	std::string name3 = renamer.ResolveUniqueName("meta", "stem", ".mkv", "meta.mkv", usedPaths, workingDir);
	BOOST_CHECK_EQUAL(name3, "meta(3).mkv");
	usedPaths.insert(workingDir / name3);

	std::string sub1 = renamer.ResolveUniqueName("meta", "eng", ".srt", "meta.srt", usedPaths, workingDir);
	BOOST_CHECK_EQUAL(sub1, "meta.srt");
	usedPaths.insert(workingDir / sub1);

	std::string sub2 = renamer.ResolveUniqueName("meta", "eng", ".srt", "meta.srt", usedPaths, workingDir);
	BOOST_CHECK_EQUAL(sub2, "meta.eng.srt");
	usedPaths.insert(workingDir / sub2);

	std::string sub3 = renamer.ResolveUniqueName("meta", "eng", ".srt", "meta.srt", usedPaths, workingDir);
