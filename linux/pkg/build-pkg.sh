...
if [ ! -f "/tmp/$INSTALLER" ]; then
    curl -L -o "/tmp/$INSTALLER" "https://github.com/nzbgetcom/nzbget/releases/download/testing/$INSTALLER"    
fi

# extract version
VERSION=$(bash "/tmp/$INSTALLER" --help | grep 'Installer for' | cut -d ' ' -f 3 | sed -r 's/nzbget-//')
RPM_VERSION=${VERSION//-/}

for ARCH in $ARCHS; do
    bash "/tmp/$INSTALLER" --arch "$ARCH" --destdir "$PWD/$ARCH" --silent    
    case $ARCH in
        i686)
            DPKG_ARCH=i386
            RPM_ARCH=i386
    ...
    if [ "$DEB" == "yes" ]; then
        ...
        cp ../linux/pkg/deb/DEBIAN/* "$CONTENTS/DEBIAN/"
