 ...
 * 
 * Consolidates extension checks (archives, videos, subtitles, etc.) to avoid
 * scattered ad-hoc lists across the codebase. All checks use std::any_of
 * on constexpr arrays for optimal performance.
 */
namespace FileTypes
{
...
// Generic numeric split volumes (.000–.999)
bool IsNumericVolumeExt(std::string_view ext);

// Any supported archive format (composes the above plus .xz)
