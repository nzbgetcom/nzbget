...
{
	if (file->GetParFile() || Util::MatchFileExt(file->GetFilename(), g_Options->GetHardLinkingIgnoreExt(), ","))
	{
		return;
	}
	...
	const std::string finalOutputFilename = finalDir + PATH_SEPARATOR + file->GetFilename();

	nzbInfo->PrintMessage(Message::mkInfo,
		"HardLinking in-progress file %s to %s",
		file->GetFilename(), finalOutputFilename.c_str()
	);

	fs::create_hard_link(file->GetOutputFilename(), finalOutputFilename, ec);
	if (ec)
	{
		nzbInfo->PrintMessage(Message::mkError,
			"Could not create hardlink %s to %s: %s",
			file->GetFilename(), finalOutputFilename.c_str(), ec.message().c_str()
		);
		return;
	}

	file->SetHardLinkPath(finalOutputFilename);
}

void DirectRenamer::HardLinkFile(NzbInfo* nzbInfo, CompletedFile* file)
{
	if (file->GetParFile() || g_Options->GetInterDirPath().empty() || Util::MatchFileExt(file->GetFilename(), g_Options->GetHardLinkingIgnoreExt(), ","))
	{
		return;
	}

	const std::string destDir = nzbInfo->GetDestDir();
	const std::string oldOutputFilename = destDir + PATH_SEPARATOR + file->GetFilename();
	const std::string finalDir = nzbInfo->BuildFinalDirName().Str();
	const std::string finalOutputFilename = finalDir + PATH_SEPARATOR + file->GetFilename();
	...
	fs::error_code ec;
	fs::create_hard_link(oldOutputFilename, finalOutputFilename, ec);
	if (ec)
	{
		nzbInfo->PrintMessage(Message::mkError,
			"Could not create hardlink %s to %s: %s",
