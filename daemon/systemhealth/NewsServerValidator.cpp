...
{
	if (!m_server.GetActive())
	{
		return Status::Info("Server is disabled.");
	}
...
Status ServerPasswordValidator::Validate() const { return CheckPassword(m_server.GetPassword()); }
...
Status ServerCipherValidator::Validate() const
{
	...
	if (!tls)
	{
		return Status::Warning("Cipher specified but TLS is disabled.");
	}

	if (cipher.size() > 256)
	{
		return Status::Warning("Cipher string is unusually long.");
	}
...
Status ServerCertVerificationValidator::Validate() const
{
	...
	if (certLevel == Options::ECertVerifLevel::cvMinimal)
	{
		return Status::Info("Certificate verification is 'Minimal'.");
	}

	if (certLevel == Options::ECertVerifLevel::cvStrict)
	{
		return Status::Ok();
	}

	return Status::Error("Unhandled or invalid certificate verification level provided.");
}
