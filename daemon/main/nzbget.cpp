...
void FSTests()
{
	std::cout << "Starting Boost.Filesystem Tests..." << std::endl;

	try {
		// 1. Current Path
		const fs::path currPath = fs::current_path();
		std::cout << "1. Current path: " << currPath << std::endl;
		if (currPath.empty()) {
			std::cerr << "Error: Current path is empty!" << std::endl;
		}

		// 2. Directory Existence
		if (fs::exists(currPath)) {
			std::cout << "2. Current path exists." << std::endl;
		}
		else {
			std::cerr << "Error: Current path does not exist!" << std::endl;
		}

		// 3. Creating and Removing a Directory
		fs::path testDir = currPath / "boost_fs_test_dir";
		if (fs::create_directory(testDir)) {
			std::cout << "3. Created directory: " << testDir << std::endl;
			if (fs::exists(testDir)) {
				std::cout << "   Directory exists after creation." << std::endl;
			}
			else {
				std::cerr << "   Error: Directory does not exist after creation!" << std::endl;
			}

			// Test removal
			if (fs::remove(testDir)) {
				std::cout << "   Removed directory: " << testDir << std::endl;
			}
			else {
				std::cerr << "   Error: Failed to remove directory: " << testDir << std::endl;
			}
		}
		else {
			std::cerr << "Error: Failed to create directory: " << testDir << std::endl;
			// Attempt to remove if it already exists, in case of prior failure
			if (fs::exists(testDir) && fs::remove(testDir)) {
				std::cout << "   Removed pre-existing directory: " << testDir << std::endl;
			}
		}

		// 4. Creating and Removing a File
		fs::path testFile = currPath / "boost_fs_test_file.txt";
		{
			std::ofstream ofs(testFile.string());
			if (ofs) {
				ofs << "This is a test file.\n";
				ofs.close();
				std::cout << "4. Created file: " << testFile << std::endl;

				if (fs::exists(testFile)) {
					std::cout << "   File exists after creation." << std::endl;
				}
				else {
					std::cerr << "   Error: File does not exist after creation!" << std::endl;
				}
				...
				if (fs::remove(testFile)) {
					...
					std::cerr << "   Error: Failed to remove file: " << testFile << std::endl;
				}
			}
			else {
				std::cerr << "Error: Failed to create file: " << testFile << std::endl;
			}
		}
		...
		// 5. File Size
		fs::path smallFile = currPath / "small_test_file.bin";
		{
			std::ofstream ofs(smallFile.string(), std::ios::binary);
			if (ofs) {
				char data[] = { 0x01, 0x02, 0x03, 0x04, 0x05 };
				ofs.write(data, sizeof(data));
				ofs.close();
				std::cout << "5. Created small file for size check: " << smallFile << std::endl;

				if (fs::exists(smallFile)) {
					boost::uintmax_t fileSize = fs::file_size(smallFile);
					std::cout << "   File size of " << smallFile << ": " << fileSize << " bytes." << std::endl;
					if (fileSize != sizeof(data)) {
						std::cerr << "   Error: Unexpected file size!" << std::endl;
					}
				}
				else {
					std::cerr << "   Error: Small file does not exist for size check!" << std::endl;
				}
			}
			else {
				std::cerr << "Error: Failed to create small file for size check: " << smallFile << std::endl;
			}
		}
		if (fs::exists(smallFile)) {
			fs::remove(smallFile);
			std::cout << "   Cleaned up small file: " << smallFile << std::endl;
		}

		// 6. Iterating through a Directory (if it contains items)
		std::cout << "6. Iterating through current directory contents (first few):" << std::endl;
		int count = 0;
		...
		if (count == 0) {
			std::cout << "   Current directory is empty or could not be iterated." << std::endl;
		}


		// 7. Path Manipulation (Joining Paths)
		fs::path dirPart = "my_data";
		fs::path filePart = "report.csv";
		fs::path combinedPath = currPath / dirPart / filePart;
		std::cout << "7. Combined path: " << combinedPath << std::endl;
		if (combinedPath.string().find(dirPart.string()) == std::string::npos ||
			combinedPath.string().find(filePart.string()) == std::string::npos) {
			std::cerr << "   Error: Path combination might be incorrect!" << std::endl;
		}

		// 8. Checking File Type
		fs::path dummyFile = currPath / "fs_test_dummy.tmp";
		{
			std::ofstream ofs(dummyFile.string());
			if (ofs) {
				ofs.close();
				std::cout << "8. Created dummy file for type check: " << dummyFile << std::endl;
				if (fs::is_regular_file(dummyFile)) {
					std::cout << "   '" << dummyFile << "' is a regular file." << std::endl;
				}
				else {
					std::cerr << "   Error: '" << dummyFile << "' is not detected as a regular file!" << std::endl;
				}
				if (fs::is_directory(dummyFile)) {
					std::cerr << "   Error: '" << dummyFile << "' is incorrectly detected as a directory!" << std::endl;
				}
			}
			else {
				std::cerr << "Error: Failed to create dummy file for type check: " << dummyFile << std::endl;
			}
		}
		if (fs::exists(dummyFile)) {
			fs::remove(dummyFile);
			std::cout << "   Cleaned up dummy file: " << dummyFile << std::endl;
		}

		// 9. Permissions (basic check - does it exist and can we potentially interact?)
		// This is hard to test universally without creating files with specific permissions.
		// A simple check is if we can obtain file status.
		if (fs::exists(currPath)) {
			try {
				fs::file_status status = fs::status(currPath);
				if (fs::exists(status)) {
					std::cout << "9. Successfully obtained status for current path." << std::endl;
				}
				else {
					std::cerr << "   Error: Could not obtain status for current path." << std::endl;
				}
			}
			catch (const fs::filesystem_error& e) {
				std::cerr << "   Error getting file status: " << e.what() << std::endl;
			}
		}


		// 10. Creating and removing a directory with spaces/special chars (platform dependent)
		// Be cautious with this as it might fail on some OS/filesystems if not handled carefully.
		// For basic tests, sticking to alphanumeric is safer.
		// Let's try a slightly more complex, but generally safe, path component.
		fs::path dirWithHyphen = currPath / "my-test-dir";
		if (fs::create_directory(dirWithHyphen)) {
			std::cout << "10. Created directory with hyphen: " << dirWithHyphen << std::endl;
			if (fs::remove(dirWithHyphen)) {
				std::cout << "    Removed directory with hyphen." << std::endl;
			}
			else {
				std::cerr << "    Error: Failed to remove directory with hyphen: " << dirWithHyphen << std::endl;
			}
		}
		else {
			std::cerr << "Error: Failed to create directory with hyphen: " << dirWithHyphen << std::endl;
			if (fs::exists(dirWithHyphen) && fs::remove(dirWithHyphen)) {
				std::cout << "    Removed pre-existing directory with hyphen." << std::endl;
			}
		}
		...
		std::cerr << "Boost.Filesystem Exception Caught: " << e.what() << std::endl;
	}
	...
	std::cout << "Boost.Filesystem Tests Finished." << std::endl;
}
