...
namespace
{
	RenameResult RenameFilesInternal(Thread& thread, PostInfo* postInfo, bool runDownloaded, bool runExtracted)
	{
		...
		std::vector<Candidate> allCandidates = CollectCandidates(thread, destPath);
		{
			...
			{
				if (nzbInfo->IsDownloadedFilename(candidate.filename))
				{
					if (runDownloaded)
					{
...
{
	RenameResult result = RenameFilesInternal(thread, postInfo,
		scope == Scope::Downloaded, scope == Scope::Extracted);
	return scope == Scope::Downloaded ? result.downloadedCount : result.extractedCount;
}
