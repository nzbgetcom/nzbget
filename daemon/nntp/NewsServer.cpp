...
NewsServer::NewsServer(int id, bool active, const char* name, const char* host, int port, int ipVersion,
	...
	int maxConnections, int pipelineDepth, int retention, int level, int group, bool optional, unsigned int certVerificationfLevel) :
		...
		m_cipher(cipher ? cipher : ""), m_maxConnections(maxConnections), m_pipelineDepth(pipelineDepth), m_retention(retention),
